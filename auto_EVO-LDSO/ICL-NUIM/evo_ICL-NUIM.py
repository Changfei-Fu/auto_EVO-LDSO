...
def kft_Gen(data_Path,Index):
    ...
    if not os.path.exists(results_Path+'/KeyFrameTrajectory'+str(Index)+'.txt'): 
        ...
        cmd="mv ./results.txt ./KeyFrameTrajectory.txt && rm ./results.txt.noloop"
        if os.path.exists("./results.txt") :
            os.system(cmd)
        else : exit()
        os.system("./modify_Spaces")

        os.system("cp ./src/rgb.txt ./")
        imagenum=len(os.listdir(data_Path+"/data"))
        os.system("./prepare "+str(imagenum)+" ./rgb.txt KeyFrameTrajectory.txt")
...
def evo_data(data_Path,times):#输入某个数据集地址
    ...
    if not os.path.exists("./prepare") or \
       not os.path.exists("./tracking_Rate") or \
       not os.path.exists("./modify_rgb") or \
       not os.path.exists("./modify_Spaces")  :
    ...
    or not os.path.exists(results_Path+"/groundtruth.txt"):
        ...
        os.system("mv "+data_Path+"/rgb "+data_Path+"/data")
        imagenum=len(os.listdir(data_Path+"/data"))
        ...
        for i in range(0,len(Names)):
            if Names[i].find("gt")>0:
                ...
                os.system("cp ./src/rgb.txt ./")
                ##find()返回起始index
                if Names[i].find("traj") >= 0 :
                    os.system("./prepare "+str(imagenum)+" ./rgb.txt groundtruth.txt 1")
                else:
                    os.system("./prepare "+str(imagenum)+" ./rgb.txt groundtruth.txt 0")

        
        os.system("./modify_rgb ./rgb.txt")

        os.system("mv ./rgb.csv "+results_Path+"/")
        os.system("mv ./groundtruth.txt "+results_Path+"/")
    
        os.system("mv ./data.csv "+data_Path+"/data.csv")
