...
def evo_data(data_Path,times):#输入某个数据集地址
    ...
    if not os.path.exists(data_Path+"/cam0/data"):
        os.system("mv "+data_Path+"/cam0/rgb "+data_Path+"/cam0/data")

    if not os.path.exists(results_Path+"/rgb.txt"):
        os.system("cp "+data_Path+"/rgb.txt "+" ./rgb.txt")
        #os.system("./modify_rgb")
        os.system("cp ./rgb.csv "+results_Path+"/rgb.csv")
        os.system("cp ./rgb.csv "+data_Path+"/data.csv")
    ...
    if os.path.exists(data_Path+"/sensor.yaml"):
